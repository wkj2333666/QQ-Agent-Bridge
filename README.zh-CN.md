...
forget 硬删除记录；变更只能使用经过验证的 revise、contradict 或 merge，硬删除仅由确定性
用户命令和存储层到期维护执行。JSON 任意层级的重复键和非标准数值常量都会被拒绝；凭据
检查会执行 Unicode NFKC、移除格式控制字符并做大小写折叠，同时覆盖多语言赋值表达和
常见秘密环境变量名。
受限 home 和 workspace 会在释放、App 关闭和启动失败时按所有权边界清理。数据库故障
只会停用长期记忆，不影响普通聊天、任务、schedule 或 OneBot 启动。

普通与主动发言 Agent trace 都会脱敏本次检索到的长期记忆条目，但不会改写最终发送到
QQ 的回答。CQ 字符串中的真实 at 与 schedule 持久化的 mention 会保留结构化检索权限；
纯展示文本不会因此获得权限。
