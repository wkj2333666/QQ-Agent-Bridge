...
任务工具，也不会向 QQ 发送中间进度。每个模型建议都必须引用当前批次的来源行，并在
引号、否定、示例、假设和拒绝记忆指令之外获得确定性的肯定支持。curator 永远不能通过
forget 或 merge 硬删除记录；后台变更只能使用经过验证的 revise 或 contradict，硬删除仅由
确定性用户命令和存储层到期维护执行。ACTIVE 建议要求引用来源除简单说话人和标点包装外，
基本就是建议中的断言。JSON 任意层级的重复键、非标准数值常量和缺少操作必填字段的响应
都会被拒绝；凭据与敏感信息检查会执行 Unicode NFKC、移除不可见格式字符和组合控制字符并
做大小写折叠，同时拒绝多语言认证标签与秘密形态值的组合及常见秘密环境变量名。
受限 home 和 workspace 会在释放、App 关闭和启动失败时按所有权边界清理。数据库故障
只会停用长期记忆，不影响普通聊天、任务、schedule 或 OneBot 启动。

普通与主动发言 Agent trace 都会脱敏本次检索到的长期记忆条目及其大小写、空白变体，但
不会改写最终发送到 QQ 的回答。CQ 字符串中的真实 at 与 schedule 持久化的 mention 会
保留结构化检索权限；纯展示文本不会因此获得权限。
