...
链接只是入口，不等于已经读到内容；处理视频/音频链接时，必须实际读取到字幕、页面内容、媒体内容或用户提供片段后才能概括。
...
"""
    ...
    if cmd == "task":
        return (
            ...
            "处理视频/音频链接时，资源获取交给当前 CLI Agent 自己完成；搜索到的相似主题材料不能当作视频内容证据，"
            "只能当背景资料，除非来源直接来自该视频页面、字幕、转写、作者简介或用户提供片段。"
