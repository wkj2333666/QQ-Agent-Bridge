...
| 延续历史任务、检索既有进度、保存可复用工作记录 | `skills/qq-agent-runtime/references/memory-tools.md` |
...
- 视频/音频链接理解类：页面元数据、简介或页面正文只能用于识别资源，不能作为正片内容证据；内容主张必须基于实际取得的字幕、转写、音频、抽帧画面、实际媒体或用户提供片段之一，不能只凭标题、短链文本、搜索片段或常识推断视频正片内容。
- 视频/音频访问受阻：登录、cookie 缺失、403、429、地域限制、限流或反爬都是阻塞；不得绕过访问控制或伪造 cookie/会话，不作视频内容概括，只报告已验证元数据和阻塞点。
- 视频/音频搜索边界：搜索到的相似主题文章、新闻、科普资料、同名标题或背景知识，以及该视频页面、作者简介等页面文字，均不能当作视频内容证据；只能用于识别资源，或只能当背景资料并标注“未验证为视频内容”。内容主张仍只能基于实际取得的字幕、转写、音频、抽帧画面、实际媒体或用户提供片段。
...
- 视频/音频理解完成：实际工具成功读取到字幕/转写、音频、画面/实际媒体或用户提供片段之一；页面元数据、简介或页面正文不能单独作为正片内容证据；否则只能说明无法概括正片，并列出已验证的元数据。
