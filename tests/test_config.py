...
""",
    ...
    assert cfg.command_groups == {
        "3000000001": {"task": "disabled", "search": "owner"}
    }
    assert cfg.command_access("task", "3000000001") == "disabled"
    assert not cfg.is_command_allowed("task", "3000000001")
    assert cfg.command_access("search", "3000000001") == "owner"
    assert cfg.command_access("ask", "3000000001") == "user"
