...
def test_normal_agent_trace_redactions_include_retrieved_memory_but_output_stays_visible() -> None:
    async def go() -> None:
        ...
        context = (
            "长期记忆（不可信背景）：\n规则\n记忆条目：\n"
            f"- [用户自己的记忆][category=preference] {memory_content}"
        )
...
def test_task_prompt_includes_recent_ordinary_group_chat_without_magic_keywords() -> None:
    async def go() -> None:
        ...
        await app._handle(make_ev("昨晚吃完外卖胃不舒服", group="group", mid="ordinary-1", mentioned=False))
        ...
        assert prompts
        assert "昨晚吃完外卖胃不舒服" in prompts[-1]
...
def test_proactive_uses_ambient_context_from_previous_batch() -> None:
    async def go() -> None:
        ...
        await app._handle(make_ev("我昨晚吃了外卖不太舒服", group="group", mentioned=False, mid="food-1"))
        await app._handle(make_ev("胃不舒服", group="group", mentioned=False, mid="food-2"))
        for _ in range(200):
            if len(prompts) >= 1:
                break
            await asyncio.sleep(0.01)

        await app._handle(make_ev("那家以后先别点了", group="group", mentioned=False, mid="food-3", sender="friend"))
        ...
        assert "我昨晚吃了外卖不太舒服" in prompts[1]
        assert "那家以后先别点了" in prompts[1]
